...
export class InvitetogameController {
  ...
  getReceivedInvitationById(
    ...
    return this.invitetogameService.getReceivedInvitationById(
      userId,
      invitationId,
    );
  }
  ...
  editInvitationById(
    @GetUser('id') userId: number,
    @Param('id', ParseIntPipe) invitationId: number,
    @Body() dto: EditInvitationDto,
  ) {
    return this.invitetogameService.editInvitationById(
      userId,
      invitationId,
      dto,
    );
