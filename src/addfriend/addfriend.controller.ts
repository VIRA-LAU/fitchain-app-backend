...
export class AddfriendController {
    ...
    @Post()
    createRequest(@GetUser('id') userId:number, @Body() dto:CreateFriendRequestDto){
        return this.addfriendService.createRequest(userId,dto)
    }

    @Patch(':id')
    editRequestById(@GetUser('id') userId:number, @Param('id', ParseIntPipe) requestId: number,@Body() dto:EditFriendRequestDto){
