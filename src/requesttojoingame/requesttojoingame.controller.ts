...
export class RequesttojoingameController {
  ...
  getReceivedRequestById(
    ...
    return this.requesttojoingameService.getReceivedRequestById(
      userId,
      requestId,
    );
  }
  ...
  createRequest(
    @GetUser('id') userId: number,
    @Body() dto: CreateRequestToJoinDto,
  ) {
  ...
  editRequestById(
    @GetUser('id') userId: number,
    @Param('id', ParseIntPipe) requestId: number,
    @Body() dto: EditRequestToJoinDto,
  ) {
    return this.requesttojoingameService.editRequestById(
      userId,
      requestId,
      dto,
    );
